...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if(transaction.height >= 0) {
    confirmations = this.node.services.trivechaind.height - transaction.height + 1;
  }
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (output.address) {
    ...
    var address = trivechaincore.Address(output.address); //TODO return type from trivechaincore-node
...
TxController.prototype.sendix = function(req, res) {
	...
	var options = {allowAbsurdFees: false, isDirectSend: true};
